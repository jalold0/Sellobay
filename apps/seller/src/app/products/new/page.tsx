    ...
    (async () => {
      try {
        ...
        if (!cancelled && json.success && json.data) {
          ...
          toast({ title: json.error?.message ?? 'Kategoriyalar yuklanmadi', variant: 'destructive' });
        }
      } catch {
        if (!cancelled) toast({ title: 'Kategoriyalar yuklanmadi (tarmoq)', variant: 'destructive' });
      }
    })();
    return () => {
      cancelled = true;
